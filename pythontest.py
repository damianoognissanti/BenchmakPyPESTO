...
for testCaseIndex in range(0,len(dataFrametestCase)):
    ...
    for ro in range(0,len(petabProblem.x_free_ids)):
        for co in range(0,len(petabProblem.x_free_ids)):
            parameterName = petabProblem.x_free_ids[ro] + petabProblem.x_free_ids[co]
            parameterValue = dataFrametestCaseHess[parameterName]
            rowind = petabProblem.x_free_indices[ro]
            colind = petabProblem.x_free_indices[co]
            JuliaHess[rowind,colind] = parameterValue[testCaseIndex]           
